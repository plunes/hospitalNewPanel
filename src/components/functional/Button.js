...
const Button = (props) => {
    if(props.icon==="download"){
        ...
        return ( <button id={props.id} style={{...props.style}} onClick = {(e)=>props.onClick(e)} className={`btn_rish btn_rish-3 btn_rish-sep icon-delete ${props.className}`}>{props.children}</button>)
        ...
        return   ( <button  style={{...props.style}} onClick = {(e)=>props.onClick(e)} className={`btn_rish btn_rish-2 btn_rish-sep no_icon_btn ${props.className}`}>{props.children}</button>)
    } else {
        return (
            <React.Fragment>
